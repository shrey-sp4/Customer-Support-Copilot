...
def main():
    ...
    try:
        ...
        keys = [
            ...
            "AvgLatencyMs", "AvgSearch_ms", "AvgFractionKBScanned", "REE@5"
        ]
        ...
        logger.info("\n--- METRIC SUMMARY ---")
        header = f"{'label':<12} | EvHit5 | WDDom  | Unsupp | TriAcc | MacF1  | FAccEx | GA_Ans | GA_Tkt | LatMs | FracKB | REE@5"
        print(header)
        print("-" * len(header))
        for r in rows:
            print(f"{r.get('label'):<12} | {r.get('EvidenceHit@5',0):.3f} | "
                f"{r.get('WrongDomainCitationRate',0):.3f} | {r.get('UnsupportedAnswerRate',0):.3f} | "
                f"{r.get('TriageAccuracy',0):.3f} | {r.get('MacroF1',0):.3f} | "
                f"{r.get('FalseAcceptRateExplicit',0):.3f} | {r.get('GoldAnswerAnsweredRate',0):.3f} | "
                f"{r.get('GoldAnswerTicketRate',0):.3f} | {r.get('AvgLatencyMs',0):.1f} | "
